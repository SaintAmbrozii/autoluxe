...
public class XlsExporter implements IExporter{
    ...
    public byte[] export(List<Payments> payments, boolean withDates) throws Exception {
        ...
        for (int i = 0; i < payments.size(); i++) { //обходим список платежек
            ...
            createStringCell(wb, row, 0, payment.getUserEmail()); //передаем в метод воркбук, строки, номер столбца, название от колони платежа
            createStringCell(wb, row, 1, payment.getUserId().toString());
            createStringCell(wb, row, 2, payment.getManagerId().toString());
            createCurrencyCell(wb, row, 3, payment.getSumma().doubleValue());
            if (withDates)
